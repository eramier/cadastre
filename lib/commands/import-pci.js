...
async function handler(archivesPath, targetPath, bundleTypeName, image) {
  ...
  const depArchives = archives.filter(f => f.match(/PCI_([\da-z]{2,3})_[\da-z_-]*.zip$/i))
  ...
  await bluebird.mapSeries(depArchives, async depArchive => {
    // eslint-disable-next-line unicorn/better-regex
    let codeDep = depArchive.match(/PCI_([\da-z]{2,3})_[\da-z_-]*.zip$/i)[1]
    if (!(codeDep.startsWith('97'))) {
      codeDep = codeDep.slice(-2)
    }
